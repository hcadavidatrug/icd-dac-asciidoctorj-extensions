...
mvn -e -DASCIIDOC_SOURCE_PATH=$PWD/sample-docs-norefs -DOUTPUT_PATH=/tmp/output -DBACKEND_URL=https://documentation-dashboard.herokuapp.com -DBACKEND_CREDENTIALS=user:123 -DPROJECT_NAME=$CI_PROJECT_NAME -DPIPELINE_ID=$CI_PIPELINE_ID -DDEPLOYMENT_URL=$CI_PAGES_URL -DSOURCE_URL=$CI_PROJECT_URL  -DCOMMIT_AUTHOR="'$CI_COMMIT_AUTHOR'" -DCREATION_DATE=$CI_PIPELINE_CREATED_AT -DCOMMIT_TAG=$CI_COMMIT_TAG -DLOG_TAGS_FILE_PATH=$PWD/logs.txt
